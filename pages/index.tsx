  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-white flex flex-col items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl aspect-[4/3] border-4 border-white rounded-xl overflow-hidden shadow-lg">
        <Image src={current.src} alt={current.title} fill className="object-contain" />
        <div className="absolute inset-0 flex items-center justify-between px-4">
          <button onClick={prev} className="text-white bg-black/40 hover:bg-black/60 px-4 py-2 rounded">⬅ Назад</button>
          <button onClick={next} className="text-white bg-black/40 hover:bg-black/60 px-4 py-2 rounded">Вперед ➡</button>
        </div>
      </div>
